...
class Generator(keras.utils.Sequence):
    ...
    def compute_inputs(self, image_group):
        ...
        for image in image_group:
            ...
            _new_image_group.append(image)
        image_group = _new_image_group
    ...
    def compute_targets(self, image_group, annotations_group):
        ...
        """
        # Grayscale
        __image_dim__ = len(image_group[0].shape)
        # get the max image shape
        max_shape = tuple(max(image.shape[x] for image in image_group) for x in range(__image_dim__))
